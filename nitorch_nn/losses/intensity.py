...
from nitorch_core.constants import eps, nan, inf, pi
from nitorch_fastmath import nanmin, nanmax, nansum, matvec
from nitorch_bayes.mixtures import GMM
...
def joint_hist_spline(x, y, bins=64, min=None, max=None, order=3, mask=None):
    ...
    if mask is not None:
        ...
        val_inf = torch.as_tensor(inf, **extra.backend(x))
        x = torch.where(mask, val_inf, x)
        y = torch.where(mask, val_inf, y)

    return extra.histc2(torch.stack([x, y], -1), bins, min_val, max_val,
...
class MutualInfoLoss(Loss):
    ...
    def forward(self, x, y, **overload):
        ...
        if patch_size:
            ...
            x = extra.unfold(x[:, 0], patch_size, patch_stride, collapse=True)
            y = extra.unfold(y[:, 0], patch_size, patch_stride, collapse=True)
