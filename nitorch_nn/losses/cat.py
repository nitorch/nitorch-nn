...
from nitorch_fastmath import softmax, log_softmax, dot, reduce
...
def get_prob_explicit(x, logit=False, implicit=False):
    ...
    if logit:
        return softmax(x, dim=1, implicit=[implicit, False],
                            implicit_index=0)
    else:
...
def get_logprob_explicit(x, logit=False, implicit=False):
    ...
    if logit:
        return log_softmax(x, dim=1, implicit=[implicit, False],
                                implicit_index=0)
    else:
...
def get_log_confusion(confusion, nb_classes_pred, nb_classes_ref, dim, **backend):
    ...
    if confusion is None:
        confusion = torch.eye(nb_classes_pred, nb_classes_ref,
                              **backend).exp()
    confusion = extra.unsqueeze(confusion, -1, dim)  # spatial shape
    if confusion.dim() < dim + 3:
        confusion = extra.unsqueeze(confusion, 0, 1)  # batch shape
...
def _process_weights(weighted, dim, nb_classes, **backend):
    ...
    if weighted is not False:
        ...
        if weighted.dim() == 1:
            weighted = extra.unsqueeze(weighted, -1, dim)
    ...
    def forward(self, score, truth, mask=None):
        ...
        if truth.dtype.is_floating_point:
            ...
            if weighted is True:
                ...
                dim = truth.dim() - 2
                weighted = extra.make_vector(weighted, nb_classes,
                                             **extra.backend(loss))
                weighted = extra.unsqueeze(weighted, -1, dim)
                if mask is not None:
                    weighted = weighted * mask
            ...
            if weighted is True:
                ...
                weighted = extra.make_vector(weighted, **extra.backend(score))
            else:
                weighted = None
...
class CategoricalLoss(CatDotProduct):
    ...
    def posterior(self, score):
        return softmax(score, dim=1, implicit=self.implicit,
    ...
    def forward(self, prior, obs, mask=None):
        ...
        if weighted is True:
            ...
            weighted = torch.as_tensor(weighted, **extra.backend(prior))
        else:
            weighted = None
        ...
        if nb_classes == 2:
            ...
            if mask is not None:
                ...
                reduction = 'none'
            if self.logit:
                obs = obs.to(**extra.backend(prior))
...
class DiceLoss(Loss):
    ...
    def forward(self, predicted, reference, mask=None):
        ...
        if not isinstance(weighted, bool):
            weighted = extra.make_vector(weighted, nb_classes, **backend)[None]
        ...
        if reference.dtype.is_floating_point:
            ...
            for index in range(first_index, nb_classes):
                ...
                inter = reduce.sum(pred1 * ref1, dim=spatial_dims)
                union = reduce.sum(pred1 + ref1, dim=spatial_dims)
