...
from nitorch_fastmath import softmax
...
class Accuracy(Metric):
    ...
    def forward(self, predicted, reference):
        ...
        dtype = extra.max_dtype(predicted, reference, force_float=True)

        # preprocess
        if self.logit:
            predicted = softmax(predicted, 1, (implicit, False))
            implicit = False
...
class Dice(Metric):
    ...
    def forward(self, predicted, reference):
        ...
        for label in range(first_index, nb_classes):
            ...
            inter = torch.sum(prd1 * ref1, dim=spatial_dims, dtype=dtype)
            prd1 = torch.sum(prd1, dim=spatial_dims, dtype=dtype)
            ref1 = torch.sum(ref1, dim=spatial_dims, dtype=dtype)
            union = prd1 + ref1
            loss1 = 2 * inter / union
