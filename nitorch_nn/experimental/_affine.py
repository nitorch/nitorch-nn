...
from nitorch_nn import check
from nitorch_nn.base import Module
from nitorch_nn.modules.cnn import UNet, CNN
from nitorch_nn.modules.spatial import (
    GridPull, GridExp, GridResize, AffineGrid,
    AffineExp, AffineLog, AffineClassic, AffineClassicInverse
)
from nitorch_interpol import resize_flow, identity_grid
from nitorch_register.affine import (
    affine_matmul, affine_lmdiv, affine_rmdiv, as_euclidean,
    affine_make_square, affine_basis_size,
)
from nitorch_fastmath import matvec
from nitorch_core.extra import unsqueeze, channel2last, last2channel
    ...
    def forward(self, source, target, source_seg=None, target_seg=None,
        ...
        if source_seg is not None:
            if source_seg.shape[2:] != source.shape[2:]:
                grid = resize_flow(grid, shape=source_seg.shape[2:])
...
class AffineVoxelMorph(Module):
    ...
    def exp(self, velocity, affine=None, displacement=False):
        ...
        if displacement:
            grid = grid - identity_grid(grid.shape[1:-1], **info)
    ...
    def forward(self, grid, **overload):
        ...
        if shift:
            affine_shift = torch.cat((
                ...
                -torch.as_tensor(shape, **info)[:, None] / 2),
                dim=1)
            affine_shift = as_euclidean(affine_shift)
        ...
        identity = identity_grid(shape, **info)[None, ...]
        affine = torch.matmul(igg(identity), gd(identity, grid))
        affine = affine.transpose(-1, -2) + eye(nb_dim)
        affine = affine[..., :-1, :]
        if shift:
            affine = as_euclidean(affine)
            affine = affine_matmul(affine_shift, affine)
            affine = as_euclidean(affine)
            affine = affine_rmdiv(affine, affine_shift)
        affine = affine_make_square(affine)
...
class AffineMorphFromDense(Module):
    ...
    def __init__(self, dim, group='CSO', mode='lie', encoder='leastsquares', 
        ...
        if encoder == 'leastsquares':
            ...
            nb_prm = affine_basis_size(group, dim)
            self.cnn = CNN(dim,
