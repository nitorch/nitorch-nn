...
from nitorch_nn.modules.cnn import UNet
from nitorch_nn.modules.spatial import GridPull, GridPush, GridExp
from nitorch_nn.modules.registration import VoxelMorph
from nitorch_core.broadcast import expand
from nitorch_core.py import make_list
from nitorch_core.extra import channel2last, last2channel
from nitorch_interpol import identity_grid
...
class IterativeVoxelMorph(VoxelMorph):
    ...
    def forward(self, source, target, *, _loss=None, _metric=None):
        ...
        grid = identity_grid(shape, dtype=dtype, device=device)
        grid = grid[None, ...]
        ...
        for n_iter in range(max_iter):
            ...
            for f in self.feed:
                if f == 'source':
                    ...
                    input_unet += [last2channel(velocity)]
                else:
            ...
            increment = channel2last(increment)
            if self.residual:
