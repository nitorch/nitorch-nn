...
from nitorch_interpol import (
    pull, resize, reslice, add_identity_grid_, identity_grid, affine_grid
)
from nitorch_register.affine import (
    voxel_size, affine_reorient, affine_matvec
)
            ...
            if isinstance(module, Conv):
                ...
                try:
                    beta = torch.as_tensor(f[key][key]['beta:0'],
                                       **extra.backend(module.norm.bias))
                except:
                ...
                try:
                    gamma = torch.as_tensor(f[key][key]['gamma:0'],
                                        **extra.backend(module.norm.weight))
                except:
                ...
                try:
                    mean = torch.as_tensor(f[key][key]['moving_mean:0'],
                                       **extra.backend(module.norm.running_mean))
                except:
                ...
                try:
                    var = torch.as_tensor(f[key][key]['moving_variance:0'],
                                      **extra.backend(module.norm.running_var))
                except:
...
class SynthPreproc:
    ...
    @classmethod
    def preproc_(cls, x):
        mn, mx = extra.quantile(x, [0.005, 0.995], keepdim=True).unbind(-1)
    ...
    def reorient(cls, x, affine, target='RAS'):
        affine, x = affine_reorient(affine, x, target)
        return x, affine

    @classmethod
    def resize(cls, x, affine, target_vx=1):
        target_vx = extra.make_vector(target_vx, x.dim(), **extra.backend(affine))
        vx = voxel_size(affine)
        factor = vx / target_vx
        fwhm = 0.25 * factor.reciprocal()
        fwhm[factor > 1] = 0
        x = smooth(x, fwhm=fwhm.tolist(), dim=3)
        x, affine = resize(x[None, None], factor.tolist(), affine=affine)
        x = x[0, 0]
        return x, affine
...
class SynthSegmenter(SynthSegUNet):
    ...
    def forward(self, x, affine=None):
        ...
        if affine is not None:
            affine, x = affine_reorient(affine, x, 'RAS')
            vx = voxel_size(affine)
            fwhm = 0.25*vx.reciprocal()
            fwhm[vx > 1] = 0
            x = smooth(x, fwhm=fwhm.tolist(), dim=3)
            x, affine = resize(x[None, None], vx.tolist(),
                                       affine=affine)
            x = x[0, 0]
        oshape = x.shape
...
class SynthMorphRegister(SynthMorphNet):
    ...
    def load(self, x, affine=None):
        ...
        if affine is not None:
            affine, x = affine_reorient(affine, x, 'RAS')
            vx = voxel_size(affine)
            x, affine = resize(x[None, None], vx.tolist(), affine=affine)
            x = x[0, 0]
    ...
    def forward(self, source, target, source_affine=None, target_affine=None):
        ...
        source = reslice(source, source_affine,
        ...
        vel = vel[0]
        grid = grid[0]
        grid -= identity_grid(grid.shape[:-1],
                                      dtype=grid.dtype,
                                      device=grid.device)
        right_affine = target_affine.inverse() @ target_affine_orig
        right_affine = affine_grid(right_affine, target_orig.shape)
        grid = pull(grid, right_affine, bound='nearest', extrapolate=True)
        grid = extra.movedim(grid, 0, -1).add_(right_affine)
        left_affine = source_affine_orig.inverse() @ target_affine
        grid = affine_matvec(left_affine, grid)
        warped = pull(source_orig.unsqueeze(-1), grid).squeeze(-1)
