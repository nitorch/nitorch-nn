...
from nitorch_nn.base import nitorchmodule, Module, Sequential
from nitorch_nn.activations import make_activation
...
class Stitch(Module):
    ...
    def forward(self, input):
        ...
        input = torch.matmul(self.weight.t(), input.unsqueeze(-1)).squeeze(-1)
        input = extra.movedim(input, -1, 1).reshape([batch, out_channels, *spatial])
