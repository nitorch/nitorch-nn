...
from nitorch_interpol import add_identity_grid_
from nitorch_nn import check
from nitorch_nn.base import Module
from nitorch_nn.activations import SoftMax
...
class VoxelMorph(Module):
    ...
    def forward(self, source, target, source_seg=None, target_seg=None,
        ...
        velocity = extra.channel2last(velocity)
...
class AtlasMorph(Module):
    ...
    def __init__(self, dim, unet=None, pull=None, exp=None, template=None):
        ...
        exp.setdefault('factor', 1)
        do_shoot = exp.pop('shoot')
        downsample_vel = extra.make_vector(exp.pop('downsample'), dim).tolist()
        vel_inter = exp['interpolation']
        vel_bound = exp['bound']
        if do_shoot:
            exp.pop('interpolation')
            ...
            if image.dtype.is_floating_point:
                ...
                for soft, label in enumerate(one_hot_map):
                    ...
                    if len(label) == 1:
                        ...
                        self.template.data[soft] += extra.isin(image, label).sum(0)[0]
            n += image.shape[0]
...
class _ConditionalAtlasMorph(AtlasMorph):
    ...
    def forward(self, target, meta, *, _loss=None, _metric=None):
        ...
        velocity0 = extra.channel2last(velocity0)
...
def registration_board(
    ...
    try:
        from nitorch_plot import get_orthogonal_slices, get_slice
        from nitorch_plot.colormaps import prob_to_rgb, intensity_to_rgb, disp_to_rgb
    ...
    *outputs, vel = outputs
    ...
    fig = plt.figure()
    if is2d:
        vel = get_slice(extra.movedim(vel[0], -1, 0))
        vel = disp_to_rgb(vel, amplitude='saturation')
        plt.imshow(vel.detach().cpu())
        plt.axis('off')
    else:
        vel = get_orthogonal_slices(extra.movedim(vel[0], -1, 0))
