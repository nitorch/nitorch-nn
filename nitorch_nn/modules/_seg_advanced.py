...
from nitorch_fastmath import lmdiv, logsumexp
from nitorch_core import extra
from nitorch_interpol import identity_grid, affine_grid, pull
from nitorch_register.affine import (
    voxel_size, affine_lmdiv, affine_matmul, affine_inv, affine_matvec
)
...
class MeanSpaceNet(Module):
    ...
    def __init__(self,
        ...
        self.vx = voxel_size(common_space[0])
    ...
    def forward(self, image, mat_native, ix_ref, ref=None, *, _loss=None, _metric=None):
        ...
        if self.coord_conv:
            ...
            id = identity_grid(self.mean_dim, dtype=inputs.dtype, device=inputs.device)
            id = id.unsqueeze(0)
            if self.dim == 3:
    ...
    def compose(self, orient_in, deformation, orient_mean, affine=None, orient_out=None, shape_out=None):
        ...
        orient_in, affine, deformation, orient_mean, orient_out \
            = extra.to_max_backend(orient_in, affine, deformation, orient_mean, orient_out)
        backend = extra.backend(deformation)
        eye = torch.eye(4, **backend)

        # Compose deformation on the right
        right_affine = affine_lmdiv(orient_mean, orient_out)
        if not (shape_mean == shape_out and right_affine.all_close(eye)):
            ...
            # -------
            # to displacement
            deformation = deformation - identity_grid(deformation.shape[:-1], **backend)
            trf = affine_grid(right_affine, shape_out)
            deformation = pull(deformation, trf, bound='dft', extrapolate=True)
        ...
        # -------
        left_affine = affine_matmul(affine_inv(orient_in), affine)
        left_affine = affine_matmul(left_affine, orient_mean)
        trf = affine_matvec(left_affine, trf)
