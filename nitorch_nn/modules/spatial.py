...
import torch
from nitorch_core import extra, constants
from nitorch_core.py import make_list
from nitorch_fastmath import logm, mdot
from nitorch_interpol import pull, push, pushcount, affine_grid, resize, resize_flow
from nitorch_register.shoot import shoot, shoot_approx, greens as get_greens
from nitorch_register.svf import exp as vexp
from nitorch_register.affine import (
    build_affine_basis, affine_matrix, affine_matrix_classic,
    affine_parameters_classic, affine_matmul, affine_lmdiv
)
...
class GridPull(Module):
    ...
    def forward(self, x, grid):
        ...
        """
        return pull(x.movedim(1, -1), grid, self.interpolation, self.bound,
                    self.extrapolate).movedim(-1, 1)
...
class GridPush(Module):
    ...
    def forward(self, x, grid, shape=None):
        ...
        return push(x.movedim(1, -1), grid, shape,
                    order=self.interpolation,
                    bound=self.bound,
                    extrapolate=self.extrapolate).movedim(-1, 1)
...
class GridPushCount(Module):
    ...
    def forward(self, x, grid, shape=None):
        ...
        """
        shape = shape or self.shape
        pushed = push(x.movedim(1, -1), grid, shape,
                      order=self.interpolation,
                      bound=self.bound,
                      extrapolate=self.extrapolate).movedim(-1, 1)
        count = pushcount(grid, shape,
                          order=self.interpolation,
                          bound=self.bound,
                          extrapolate=self.extrapolate).unsqueeze(1)
        return pushed, count
...
class GridExp(Module):
    ...
    def forward(self, velocity, fwd=None, inv=None):
        ...
        output = []
        if fwd:
            y = vexp(velocity, inverse=False, **opt)
            output.append(y)
        if inv:
            iy = vexp(velocity, inverse=True, **opt)
            output.append(iy)
    ...
    def forward(self, velocity, fwd=None, inv=None, voxel_size=None):
        ...
        if self.cache_greens:
            if getattr(self, '_greens_prm', None) == greens_prm:
                greens = self._greens.to(**extra.backend(velocity))
            else:
                greens = get_greens(**greens_prm, **extra.backend(velocity))
                self._greens = greens
                self._greens_prm = greens_prm
        else:
            greens = get_greens(**greens_prm, **extra.backend(velocity))

        shoot_fn = shoot_approx if self.approx else shoot
...
class AffineExp(Module):
    ...
    def __init__(self, dim, basis='CSO', fwd=True, inv=False):
        ...
        self.dim = dim
        self.basis = build_affine_basis(basis, dim)
        self.fwd = fwd
        self.inv = inv
    ...
    def forward(self, prm, fwd=None, inv=None):
        ...
        output = []
        if fwd:
            aff = affine_matrix(prm, self.basis)
            output.append(aff)
        if inv:
            if isinstance(prm, (list, tuple)):
                ...
                prm = -prm
            iaff = affine_matrix(prm, self.basis)
            output.append(iaff)
...
class AffineLog(Module):
    ...
    def __init__(self, dim, basis='CSO', **backend):
        ...
        """
        super().__init__()
        self.basis = build_affine_basis(basis, dim, **backend)
...
class AffineClassic(Module):
    ...
    def forward(self, prm):
        ...
        return affine_matrix_classic(prm, dim=self.dim)
...
class AffineClassicInverse(Module):
    ...
    def forward(self, affine):
        ...
        """
        T, R, Z, S = affine_parameters_classic(affine,
...
class AffineGrid(Module):
    ...
    def forward(self, affine, shape=None):
        ...
        if self.shift:
            ...
            affine = affine_matmul(affine, affine_shift)
            affine = affine_lmdiv(affine_shift, affine)

        grid = affine_grid(affine, shape)
        return grid
...
class Resize(Module):
    ...
    def forward(self, image, affine=None, output_shape=None):
        ...
        return resize(image, affine=affine, **kwargs)
    ...
    def shape(self, image, affine=None, output_shape=None):
        ...
        batch = inshape[:2]
        inshape = inshape[2:]
        factor = extra.make_vector(self.factor or 0., nb_dim).tolist()
...
class GridResize(Module):
    ...
    def forward(self, grid, affine=None, output_shape=None, **overload):
        ...
        return resize_flow(grid, affine=affine, **kwargs)
