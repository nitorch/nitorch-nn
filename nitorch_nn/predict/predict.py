...
import math
import torch
from nitorch_core.patches import fold, unfold
...
class SlidingWindow(Module):
    ...
    def forward(self, x, model, **fwdargs):
        ...
        x = ensure_shape(x, (x.shape[0],x.shape[1],) + tuple(pshape))
        x = unfold(x, kernel_size=self.patch_size, stride=self.stride, collapse=True)
        ...
        x = torch.cat(x, dim=2)
        x = fold(x, dim=dim, stride=self.stride, collapsed=True, shape=pshape, reduction=self.reduction)
        x = ensure_shape(x, (x.shape[0],x.shape[1],) + tuple(shape))
        return x
