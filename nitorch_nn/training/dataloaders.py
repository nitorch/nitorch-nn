...
from nitorch_core.extra import unsqueeze, movedim, one_hot, merge_labels
...
class Dataset:
    ...
    def rescale(self, image):
        ...
        if self.qmin == 0 and self.qmax == 1:
            return image
        qmin, qmax = quantile(image, [self.qmin, self.qmax])
        image -= qmin
        image /= (qmax - qmin)
        return image
    ...
    def to_shape(self, image, bound='zero'):
        ...
        if self.shape:
            ...
            return ensure_shape(image, oshape, mode=bound, side='both')
        if self.shape_min:
    ...
    def load(self, fname, dtype=None, device=None):
        ...
        dat = unsqueeze(dat, -1, max(0, dim - dat.dim()))
    ...
    def loadseg(self, fnames, segtype='label', lookup=None,
        ...
        if insegtype == 'label' and lookup:
            channels = merge_labels(channels, lookup)
        if insegtype == 'label' and outsegtype != 'label':
            channels = one_hot(channels, dim=0,
